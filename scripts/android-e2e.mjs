...
async function devtools() {
  for (let i = 0; i < 60; i++) {
    const m = adb('shell cat /proc/net/unix').match(/@(webview_devtools_remote_\d+)/);
    if (m) { adb(`forward tcp:9222 localabstract:${m[1]}`); break; }
    await sleep(2000);
  }
  for (let i = 0; i < 30; i++) {
...
class CDP {
  constructor(url) {
    this.ws = new WebSocket(url); this.id = 0; this.cb = new Map();
  ...
  send(method, params = {}) { const id = ++this.id; this.ws.send(JSON.stringify({ id, method, params })); return new Promise((r) => this.cb.set(id, r)); }
  async eval(body) {
...
const until = async (cdp, cond, ms = 60000) => { const t0 = Date.now(); while (Date.now() - t0 < ms) { try { if (await cdp.eval(`return !!(${cond})`)) return true; } catch { /* ricarica in corso */ } await sleep(700); } return false; };

let cdp;
try {
  cdp = new CDP(await devtools()); await cdp.ready;
  out.checks.avvio = await until(cdp, 'window.state && state.res && document.querySelector("#list .row")', 90000);
  shot('01-avvio');
  // guida al primo avvio: parte da sola
  out.checks.guida_parte = await until(cdp, 'window.TOUR && TOUR.el', 8000);
  await sleep(1500); shot('02-guida-benvenuto');
  // italiano (l'emulatore è in inglese), poi di nuovo la guida da capo
  await cdp.eval(`localStorage.setItem('sf.lang', JSON.stringify('it')); location.reload(); return 1`).catch(() => {});
  await sleep(4000); await until(cdp, 'window.state && state.res && document.querySelector("#list .row")', 90000);
  await cdp.eval(`if (window.TOUR && TOUR.el) tourClose(); tourStart(TOUR_STEPS, 0); return 1`); await sleep(2200); shot('03-guida-it');
  for (const [k, name] of [[1, 'luogo'], [3, 'notti'], [8, 'quanto-ci-vuole'], [11, 'cielo'], [12, 'progetti']]) {
    await cdp.eval(`tourGo(Math.min(${k}, TOUR.steps.length - 1)); return 1`); await sleep(2000); shot(`04-guida-${String(k).padStart(2, '0')}-${name}`);
  }
  await cdp.eval(`tourEnd(); return 1`); await sleep(1200);
  // meteo, preferiti, sezioni
  out.checks.meteo = await until(cdp, 'wxOk()', 60000);
  await cdp.eval(`for (const id of ['NGC 7000','IC 1396','NGC 281','IC 1805','NGC 6960']) if (state.byId.has(id) && !isFav(id)) toggleFav(id); return 1`);
  const views = [['tonight', 'stanotte'], ['targets', 'target'], ['sky', 'cielo'], ['projects', 'progetti'], ['setup', 'setup']];
  for (const [v, name] of views) { await cdp.eval(`setView('${v}'); return 1`); await sleep(v === 'projects' ? 4500 : 2500); shot(`05-${name}`); }
  await cdp.eval(`setView('tonight'); document.getElementById('v-tonight').scrollTop = 900; return 1`); await sleep(1200); shot('05-stanotte-giu');
  // dettaglio di un target: tempi e calendario
  await cdp.eval(`const id = state.byId.has('NGC 7000') ? 'NGC 7000' : state.filtered[0].o.id; openDetail(id); setDTab('piano'); return 1`); await sleep(2500);
  await cdp.eval(`const d = document.getElementById('drawer'); d.scrollTop = document.getElementById('scen').offsetTop - 120; return 1`); await sleep(1500); shot('06-dettaglio-tempi');
  await cdp.eval(`setDTab('quando'); return 1`); await sleep(2000); shot('07-dettaglio-quando');
  // tasto indietro di Android: chiude il dettaglio
  adb('shell input keyevent KEYCODE_BACK'); await sleep(1200);
  out.checks.indietro_chiude_dettaglio = await cdp.eval(`return document.getElementById('drawer').hidden`);
  shot('08-dopo-indietro');

  // avviso programmato (LocalNotifications): la prova di Setup
  await cdp.eval(`LS.set('sf.notify', true); await testNotify(); return 1`);
  await sleep(12000);
  const n1 = notifs();
  ...
  out.runner_config = await cdp.eval(`try { await ${BR}.dispatchEvent({ label: '${LABEL}', event: 'config', details: ${cfg} }); return 'ok'; } catch (e) { return 'errore: ' + e.message; }`);
  out.runner_selftest = await cdp.eval(`try { return await ${BR}.dispatchEvent({ label: '${LABEL}', event: 'selftest', details: {} }); } catch (e) { return 'errore: ' + e.message; }`);
  await sleep(4000);
  const n2 = notifs();
  out.checks.avviso_da_script = /prova e2e dallo script in background/.test(n2);
  out.runner_status = await cdp.eval(`try { return await ${BR}.dispatchEvent({ label: '${LABEL}', event: 'status', details: {} }); } catch (e) { return 'errore: ' + e.message; }`);
  ...
  out.errori_console = await cdp.eval(`return (window.__errs || []).slice(0, 10)`).catch(() => null);
} catch (e) {
